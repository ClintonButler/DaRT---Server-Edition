...
namespace DaRT
{
    public class RCon
    {
        ...
        public FollowingTail _myTail = null;
                ...
                case BattlEyeDisconnectionType.ConnectionLost:
                    if (!_reconnecting)
                    {
                        if (_form != null)
                        {
                            ...
                            _form.Log("Connection to server was lost. Attempting to reconnect...", LogType.Console, false);
                        }
                        this.Reconnect();
                    }
                    break;
                ...
                case BattlEyeDisconnectionType.SocketException:
                    if (_form.connect.Enabled)
                    {
                        if (_form != null)
                        {
                            ...
                            _form.Log("Host invalid!", LogType.Console, false);
                        }
                    }
                    else
                    {
                        if (!_reconnecting)
                        {
                            if (_form != null)
                            {
                                ...
                                _form.Log("It appears that the server went down. Attempting to reconnect...", LogType.Console, false);
                            }
                            this.Reconnect();
                        }
                    }
        ...
        {
            if (Settings.Default.killFeed && _client != null)
            {
                if (killfeedinput.Contains(" has been killed by player "))
                {
                    killfeedinput = Regex.Replace(killfeedinput, @"\(.*?\)", "").Replace(@"\s{2,}", " ").Replace("\"", "").Replace("Player ", "").Replace("player ", ""); // Remove garbage from string
        ...
        {
            /* --------------------------------------------------------------------------------------------------------------------------------------- *\
            ]* TODO: Add lines to turn on log file reader                                                                                              *[
            \* --------------------------------------------------------------------------------------------------------------------------------------- */
            if (Settings.Default.killFeed && Settings.Default.admFile != "" && Settings.Default.admFile != null)
            {
                try
                {
                    _form.Log("Loading ADM file: " + Settings.Default.admFile, LogType.Console, false);
                    _myTail = new FollowingTail(
                                               new FileInfo(Settings.Default.admFile),
                                               Encoding.ASCII,
                                               s =>
                                               {
                                                   string replacement = Regex.Replace(s, @"\t|\n|\r", "");
                                                   //_form.Log(replacement, LogType.Console, false);
                                                   HandleKillFeed(s);
                                               });
                }
                catch
                {
                    _form.Log("An error occured while accessing the ADM file.", LogType.Console, true);
                }
            }
        }
    }
