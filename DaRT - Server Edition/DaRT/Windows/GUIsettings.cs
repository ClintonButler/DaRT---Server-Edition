...
namespace DaRT
{
    public partial class GUIsettings : Form
    {
        ...
        public GUIsettings(GUImain gui)
        {
            ...
            tooltip.SetToolTip(button1, "Choose ADM File");
            tooltip.SetToolTip(showKillFeed, "If checked, DaRT will send an announcement in global chat when a player kills another player.");
            ...
            showKillFeed.Checked = Settings.Default.killFeed;
            admFile.Text = Settings.Default.admFile;
        ...
        private void done_Click(object sender, EventArgs e)
        {
            ...
            Settings.Default.killFeed = showKillFeed.Checked;
            Settings.Default.admFile = admFile.Text;
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (openFileDialogADM.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                admFile.Text = openFileDialogADM.FileName;
        }

        private void showKillFeed_CheckedChanged(object sender, EventArgs e)
        {
            if (showKillFeed.Checked)
            {
                interval.Text = "3";
                interval.Enabled = false;
                gui.update_autoRefresh(true, false);
                gui.update_messageLock(true, false);
            }
            else
            {
                interval.Enabled = true;
                gui.update_autoRefresh(true, true);
                if (announcePlayers.Checked)
                {
                    gui.update_messageLock(true, false);
                }
                else
                {
                    gui.update_messageLock(true, true);
                }
            }
        }

        private void announcePlayers_CheckedChanged(object sender, EventArgs e)
        {
            if (announcePlayers.Checked)
            {
                refreshOnJoin.Checked = true;
                refreshOnJoin.Enabled = false;
                gui.update_messageLock(true, false);
            }
            else
            {
