...
namespace DaRT
{
    public partial class GUImain : Form
    {
        ...
        public RCon rcon;
        public String lastLineProcessed, currentLine;
        ...
        private void InitializeDatabase()
        {
            ...
            {
                Log(e.Message, LogType.Debug, false);
                Log(e.StackTrace, LogType.Debug, false);
            }
        }
        ...
        private void InitializeFields()
        {
            ...
            {
                update_autoRefresh(true, false);
                update_messageLock(true, false);
            }
            else
            {
                update_autoRefresh(true, true);
                update_messageLock(true, true);
            }
        ...
        private void connect_Click(object sender, EventArgs args)
        {
            if (!pendingConnect)
            {
                ...
                Log("DaRT is already connecting. Please wait for it to finish before connecting again.", LogType.Console, false);
        }
        private void refresh_Click(object sender, EventArgs args)
        {
            if (!(tabControl.SelectedTab.Text == "Player Database"))
            {
                if (tabControl.SelectedTab.Text == "Players")
                {
                    ...
                    {
                        Log("There already is a pending player list request, please wait for it to finish!", LogType.Console, false);
                    }
                }
                else if (tabControl.SelectedTab.Text == "Bans")
                {
                    ...
                    {
                        Log("There already is a pending ban list request, please wait for it to finish!", LogType.Console, false);
                    }
                }
            }
            else
        ...
        private void disconnect_Click(object sender, EventArgs args)
        {
            ...
            Log("Disconnected.", LogType.Console, false);
        ...
        private void comment_click(object sender, EventArgs args)
        {
            ...
            try
            {
                ...
                else if (tabControl.SelectedTab.Text == "Bans")
                {
                    ...
                    {
                        Log("Comments can't be assigned to IPs.", LogType.Console, false);
                        return;
                    }
                }
            ...
            {
                Log("An error occurred, please try again.", LogType.Console, false);
                Log(e.Message, LogType.Debug, false);
                Log(e.StackTrace, LogType.Debug, false);
            }
            gui.ShowDialog();
        ...
        private void delete_click(object sender, EventArgs args)
        {
            ...
            Log("Entry was removed", LogType.Console, false);
        }
        ...
        private void expired_click(object sender, EventArgs args)
        {
            ...
            Log("All expired bans were removed from the ban list.", LogType.Console, false);
        }
        ...
        private void playerCopyAll_click(object sender, EventArgs args)
        {
            ...
            {
                Log("Error while accessing clipboard!", LogType.Console, false);
                Log(e.Message, LogType.Debug, false);
                Log(e.StackTrace, LogType.Debug, false);
            }
        }
        ...
        private void playerCopyGUID_click(object sender, EventArgs args)
        {
            ...
            {
                Log("Error while accessing clipboard!", LogType.Debug, false);
                Log(e.Message, LogType.Debug, false);
                Log(e.StackTrace, LogType.Debug, false);
            }
        }
                ...
                if (input.Text != "")
                {
                        ...
                        if (options.SelectedItem.ToString() == "Say Global")
                        {
                            ...
                            if (input.Text.Length > 3)
                            {
                                ...
                                Log("You need to enter atleast 4 characters.", LogType.Console, false);
                        }
            ...
            if (isPort(port.Text))
            {
                ...
                if (password.Text != "")
                {
                    ...
                    if (Settings.Default.showConnectMessages)
                        Log("Connecting to " + host.Text + ":" + port.Text + "...", LogType.Console, false);
                ...
                {
                    Log("Please enter a valid password!", LogType.Console, false);
                }
            }
            else
            {
                Log("Please enter a valid port!", LogType.Console, false);
            }
        ...
        public void thread_Player()
        {
            try
            {
                ...
                if (Settings.Default.showRefreshMessages)
                    Log("Refreshing players...", LogType.Console, false);
            ...
            {
                Log("Player request timed out! (Critical error)", LogType.Console, false);
                Log(e.Message, LogType.Debug, false);
                Log(e.StackTrace, LogType.Debug, false);
                pendingPlayers = false;
            }
        ...
        public void thread_Bans()
        {
            try
            {
                ...
                {
                    if (Settings.Default.showRefreshMessages)
                        Log("Refreshing bans...", LogType.Console, false);
            ...
            catch(Exception e)
            {
                if (Settings.Default.dartbrs)
                {
                    Log("This server is not running BRS or the server configuration is faulty!", LogType.Console, false);
                }
                else
                {
                    Log("Ban request timed out! (Critical error)", LogType.Console, false);
                    Log(e.Message, LogType.Debug, false);
                    Log(e.StackTrace, LogType.Debug, false);
                }
                pendingBans = false;
        ...
        private void thread_Sync()
        {
            ...
            Log("Starting player database sync...", LogType.Console, false);
            Log("This could take a while depending on how many players are in the database.", LogType.Console, false);
            Log("Please do not close DaRT during the process.", LogType.Console, false);
            dbCache.Clear();

            try
            {
                Log("Creating a backup of the existing database...", LogType.Console, false);
                if (File.Exists("data/db/players.db"))
                    File.Copy("data/db/players.db", "data/db/players.db_bak");

                Log("Reading database...", LogType.Console, false);
                command = new SqliteCommand(connection);
                ...
                Log("Wiping database...", LogType.Console, false);
                command = new SqliteCommand(connection);
                ...
                Log("Contacting master server...", LogType.Console, false);
                Log("Syncing " + sync.Count + " players from database...", LogType.Console, false);
                ...
                Log("Requesting new database...", LogType.Console, false);
                if (connection.State == ConnectionState.Open)
                {
                    ...
                    if (status == 200 && responseStream.ContentLength != 0)
                    {
                        ...
                        using (StreamReader stream = new StreamReader(responseStream.GetResponseStream()))
                        {
                            ...
                            Log("Received " + received + "players from master server.", LogType.Console, false);
                        }
                    }
                    else
                    {
                ...
                Log("Player sync complete.", LogType.Console, false);
            }
            catch(Exception e)
            {
                Log("Something went wrong.", LogType.Console, false);
                Log("Restoring backup...", LogType.Console, false);
                ...
                Log("Please try again later.", LogType.Console, false);
                Log(e.Message, LogType.Debug, false);
                Log(e.StackTrace, LogType.Debug, false);
            }
            ...
            Log("Finished sync, thank you for your support!");
             */
        }
        #endregion
        ...
        public void update_autoRefresh(bool state_checked, bool state_enabled)
        {
            autoRefresh.Checked = state_checked;
            autoRefresh.Enabled = state_enabled;
        }
        public void update_messageLock(bool state_checked, bool state_enabled)
        {
            allowMessages.Checked = state_checked;
            allowMessages.Enabled = state_enabled;
        }
        ...
        private void GUI_KeyDown(object sender, KeyEventArgs args)
        {
            if (args.KeyCode == Keys.F5)
            {
                ...
                if (!(tabControl.SelectedTab.Text == "Player Database"))
                {
                    ...
                    {
                        Log("Please connect first!", LogType.Console, false);
                    }
                }
                else
                {
        ...
        private void GUI_Load(object sender, EventArgs args)
        {
            ...
            /*
            if (Settings.Default.killFeed && rcon.Connected)
            {
                try
                {
                    new FollowingTail(new FileInfo(Settings.Default.admFile),
                        Encoding.ASCII,
                        s =>
                        {
                            this.rcon.HandleKillFeed(s);
                            Log(s, LogType.Debug, false);
                        });
                }
                catch
                {
                    Log("An error occured while accessing the ADM file.", LogType.Console, false);
                }
            }
            */
            ...
            if (Settings.Default.saveLog)
            {
                ...
                {
                    Log("An error occured while accessing the log file.", LogType.Console, false);
                    Log(e.Message, LogType.Debug, false);
                    Log(e.StackTrace, LogType.Debug, false);
                }
            }
        ...
        {
            Log("Sorting ban list is disabled in this version, sorry.", LogType.Console, false);
        ...
        {
            Log("Sorting player database is disabled in this version, sorry.", LogType.Console, false);
            /*
        ...
        private void autoScroll_CheckedChanged(object sender, EventArgs e)
        {
            if (allowMessages.Checked && _queueing && _queue.Count > 0)
            {
                ...
                {
                    Log(item.Message, item.Type, item.Important);
                }
                _queue.Clear();
            }
        }
